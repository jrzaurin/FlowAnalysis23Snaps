...
extractSeq <- function(inp,splitMove) {
    ...
    tempSeqL <- lapply(tempSeqL, function(x) trim(x))
...
##slit a vector everytime "eventname" appears and returns a list of vectors in the form
##(A B C D E A B F G A) split on A: (B C D E A) , (B F G A) 
splitlast <- function(inp.vect, eventname){
    init = 1
    r <- list()
    count = 1
    indx <- which(inp.vect == eventname)
    while(count <= length(indx)){
        if (indx[count] == 1) {
            count = count + 1
            init = init + 1
        } else {
            spl <- inp.vect[init:indx[count]]
            r <- append(r, list(spl))
            init <- indx[count] + 1    
            count = count + 1
        }
